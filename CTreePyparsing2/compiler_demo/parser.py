...
def _make_parser():
    ...
    keywords = IF | FOR | WHILE | SWITCH | CASE | DEFAULT | RETURN
    ...
    # Добавляем после других правил для statement'ов
    case_ = CASE.suppress() + expr + pp.Literal(':').suppress() + stmt_list
    default_ = DEFAULT.suppress() + pp.Literal(':').suppress() + stmt_list
    switch_case = case_ | default_
    switch_ = SWITCH.suppress() + LPAR + expr + RPAR + LBRACE + pp.ZeroOrMore(switch_case) + RBRACE
